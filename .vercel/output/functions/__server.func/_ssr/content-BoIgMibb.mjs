...
var SEED_ARTICLES = [
	{
		...
		excerpt: "A realistic, month-by-month path from your first HTML page to a deployed full-stack project — written for students starting from zero.",
...
Consistency beats intensity. Two focused hours a day for six months will outperform any weekend marathon.`,
	...
	},
	{
		...
		excerpt: "Six strong pieces beat sixty average ones. How to choose, present and explain the work that convinces a client to pay you.",
...
Replace your weakest piece every two months. A portfolio is not an archive; it is an argument that you are good right now.`,
	...
	},
	{
		...
		excerpt: "The chicken-and-egg problem of freelancing, solved: how new SkillStack graduates win their first paid project in 30 days.",
...
*/
async function safeList(table, build) {
	try {
...
var fetchArticles = async () => {
	const rows = await safeList("articles", (q) => q.select("*").eq("published", true).order("published_at", { ascending: false }));
	const slugs = new Set(rows.map((r) => r.slug));
	const extras = SEED_ARTICLES.filter((a) => !slugs.has(a.slug));
	return [...rows, ...extras].sort((a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime());
};
...
var fetchNews = () => safeList("news_items", (q) => q.select("*").order("published_at", { ascending: false }));
var fetchGallery = () => safeList("gallery_items", (q) => q.select("*").order("sort_order", { ascending: true }));
var fetchShowcase = () => safeList("showcase_projects", (q) => q.select("*").order("created_at", { ascending: false }));
var fetchEvents = () => safeList("events", (q) => q.select("*").order("starts_at", { ascending: true }));
var fetchDownloads = () => safeList("downloads", (q) => q.select("*").order("created_at", { ascending: false }));
var YOUTUBE_CHANNEL_URL = "https://www.youtube.com/@Skillstack-h2x";
...
//#endregion
export { fetchEvents as a, fetchShowcase as c, fetchDownloads as i, formatDate as l, fetchArticle as n, fetchGallery as o, fetchArticles as r, fetchNews as s, YOUTUBE_CHANNEL_URL as t };
